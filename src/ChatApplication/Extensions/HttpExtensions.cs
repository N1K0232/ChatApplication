...
public static partial class HttpExtensions
{
    public static bool IsMobileRequest(this HttpContext httpContext)
    {
        ...
        return userAgent?.Length > 4 && (MobileBrowserRegex().IsMatch(userAgent) || MobileBrowserVersionRegex().IsMatch(userAgent.AsSpan(0, 4)));
    }

    public static string GetCultureFromRoute(this HttpContext httpContext)
    {
        var request = httpContext.Request;
        var match = RouteCultureRegex().Match(request.Path);

        var culture = match.Groups["culture"];
        return culture.Value.ToLowerInvariant();
    }
    ...
    public static string GetPathWithCulture(this HttpContext httpContext, string culture)
    {
        ...
        return $"/{culture}{path}{request.QueryString}";
    }
    ...
    {
        var request = httpContext.Request;
        return $"{request.Scheme}://{request.Host}{request.PathBase}{urlHelper.Content(contentPath)}";
    }
